...
function App() {
    ...
    let binarySearch = function (arr, x, start, end) {
        ...
        if(arr[mid].id > x)
            return binarySearch(arr, x, start, mid-1); //-1
        else
            return binarySearch(arr, x, mid+1, end);//+1
    }

    const getCoordsByCityID = (id) => {
        let obj = JSON.parse(JSON.stringify(dataJSON));
        let city = binarySearch(obj, parseInt(id), 0, obj.length - 1);
        return {lon: city.coord.lon, lat: city.coord.lat}
    }
